...
<div class="flex items-start gap-3 border px-4 py-3 text-sm {{ $classes }}" role="{{ $role }}">
    <svg class="mt-0.5 h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">{!! $icon !!}</svg>
    <p class="flex-1 leading-relaxed">{{ $message }}</p>
    <button type="button" onclick="this.parentElement.remove()" class="shrink-0 rounded p-0.5 text-current/60 touch-manipulation hover:text-current focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-current/40" aria-label="Dismiss">
