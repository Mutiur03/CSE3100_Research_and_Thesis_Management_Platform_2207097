    ...
    <div class="flex min-h-screen">
        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-30 hidden w-60 flex-col border-r border-stone-200 bg-white pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] lg:flex" aria-label="Primary">
            ...
            <div class="border-t border-stone-200 bg-white p-4">
                <div class="flex items-center gap-3">
                    <img src="{{ auth()->user()->avatar_url }}" alt="" width="36" height="36" class="h-9 w-9 rounded object-cover ring-1 ring-stone-200">
        ...
        <div class="flex min-w-0 flex-1 flex-col lg:pl-60">
            <header class="sticky top-0 z-20 flex h-14 items-center justify-between border-b border-stone-200 bg-white px-4 pt-[env(safe-area-inset-top)] sm:px-6 lg:px-8">
                <div class="flex items-center gap-3">
                    <button id="sidebar-toggle" type="button" class="rounded p-2 text-stone-500 touch-manipulation hover:bg-stone-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-navy-700 lg:hidden" aria-label="Open navigation" aria-expanded="false" aria-controls="sidebar">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-semibold text-stone-800">@yield('title', 'Dashboard')</p>
                    </div>
                </div>

                <div class="flex items-center gap-3">
                    <div class="relative" id="account-menu">
                        <button type="button" data-dropdown-toggle class="flex items-center gap-2 rounded border border-stone-200 bg-white py-1.5 pl-1.5 pr-2.5 text-sm touch-manipulation hover:bg-stone-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-navy-700" aria-label="Account menu" aria-expanded="false" aria-haspopup="menu">
                            <img src="{{ auth()->user()->avatar_url }}" alt="" width="28" height="28" class="h-7 w-7 rounded object-cover">
                            <span class="hidden font-medium text-stone-700 sm:inline">{{ auth()->user()->name }}</span>
                            <svg class="h-4 w-4 text-stone-400" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" /></svg>
                        </button>
                        <div role="menu" class="absolute right-0 z-50 mt-1 hidden w-48 rounded border border-stone-200 bg-white py-1 shadow-lg dropdown-open:block">
                            <a wire:navigate.hover href="{{ route('profile.show') }}" role="menuitem" class="block px-4 py-2 text-sm text-stone-700 hover:bg-stone-50">Profile settings</a>
            ...
            @if(session('success') || session('error') || session('warning'))
                <div class="space-y-2 border-b border-stone-200 bg-stone-50 px-4 py-3 sm:px-6 lg:px-8" aria-live="polite">
                    @if(session('success'))
            ...
            <main id="main-content" class="flex-1 px-4 py-8 sm:px-6 lg:px-8" tabindex="-1">
                @yield('content')
            </main>
        </div>
    </div>
    ...
    <script>
        (function () {
            ...
            document.addEventListener('click', function (e) {
                ...
                if (dropdownToggle) {
                    const menu = dropdownToggle.parentElement;
                    const willOpen = !menu?.classList.contains('dropdown-open');
